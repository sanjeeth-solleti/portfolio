...
const IntroAnimation: React.FC<IntroAnimationProps> = ({ onAnimationComplete }) => {
  ...
  useEffect(() => {
    ...
    // Animation timing based on delays/entrance for all letters
    const entranceDelay = 0.17 * (letters.length - 1); // (per-letter delay) * (#letters - 1)
    const animateInDuration = 1.2; // animation duration
    const extraPause = 1.0; // pause after all letters in
    const timeUntilHideLetters = (entranceDelay + animateInDuration + extraPause) * 1000;

    timeout1 = setTimeout(() => {
      setLettersVisible(false); // triggers AnimatePresence exit for all letters
    }, timeUntilHideLetters);

    // Wait for exit animation to finish (0.7s), then show full name
    timeout2 = setTimeout(() => {
      setShowFullName(true);
      timeout3 = setTimeout(() => {
        setFinal(true);
        onAnimationComplete();
      }, 3200); // full name stays for 3.2s, then finish
  ...
  return (
    ...
    >
      ...
      {/* AnimatePresence guarantees visible exit animation */}
      <AnimatePresence>
        {lettersVisible && (
          <motion.h1
            key="sanjeeth"
      ...
      {/* Full name reveal in #6ee7b7 mint green */}
      {showFullName && (
        <motion.div
